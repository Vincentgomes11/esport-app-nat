...
function ChoiceInfluencer({takeToken, navigation}) {
    ...
    useEffect(() => {
        async function fetchData() {
          const response = await fetch(`https://esport-app.herokuapp.com/get-influencer-request-list?brandToken=${takeToken}`)
      ...
      const updateStatusAcc = async () => {
        ...
        const data = await fetch('https://esport-app.herokuapp.com/update-request-acc', {
          method: 'POST',
  ...
  const updateStatusRef = async () => {
    ...
    const data = await fetch('https://esport-app.herokuapp.com/update-request-ref', {
      method: 'POST',
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
      body: `token=${takeToken}`
    })
